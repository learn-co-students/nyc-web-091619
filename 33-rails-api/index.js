let api = "http://localhost:3000/api/v1/celebrities"
...
function clickHandler(e) {
    console.log("current data: ", e.target.parentNode)
    let parent = e.target.parentNode
    let id = parent.dataset.id
    let span = e.target.parentNode.querySelector("h4 span")
    let likes = parseInt(span.innerText) + 1
    fetch(`http://localhost:3000/api/v1/celebrities/${id}`, {
        method: "PATCH",
        headers: {
            "content-type": "application/json",
            accepts: "application/json"
        ...
        .then(resp => resp.json())
        .then(function () { span.innerText = likes })
...
function appendCelebrity(element) {
    ...
    h4.innerHTML = `Score: <span>${element.likes}</span>`
